#include "DEMParticleSandBuilder.h"
...
inline std::unique_ptr<ArgsParser> BuildArgsParser()
{
	...
	parser->addArgument<uint>("end", 'e', "the end frame (excluding)", 10000);
	parser->addArgument<uint>("rate", 'r', "the frame rate (frames per second)", 10000);
...
int main(int argc, char *argv[])
{
	...
	std::unique_ptr<Simulation> sand;
	if (dim == 2)
		sand = DEMParticleSandBuilder::build<2>(scale, test);
	else if (dim == 3)
		sand = DEMParticleSandBuilder::build<3>(scale, test);
	else {
		std::cerr << "Error: [main] encountered invalid dimension." << std::endl;
		std::exit(-1);
	}
	auto simulator = std::make_unique<Simulator>(output, begin, end, rate, cfl, sand.get());
	simulator->Simulate();
