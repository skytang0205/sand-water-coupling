...
namespace PhysX {
    ...
    template<int Dim> class ImplicitSurface : public Surface<Dim> {
    ...
    public:
        ...
        virtual VectorDr closestPosition(const VectorDr & pos) const {
            return pos - signedDistance(pos) * closestNormal(pos);
        }
    ...
    template<int Dim> class ImplicitSphere final : public ImplicitSurface<Dim> {
        DECLARE_DIM_TYPES(Dim)
    ...
    public:
        ...
        virtual VectorDr closestPosition(const VectorDr & pos) const override {
            return _center + closestNormal(pos) * _radius;
        }
    ...
    template<int Dim> class ImplicitBox final : public ImplicitSurface<Dim> {
        DECLARE_DIM_TYPES(Dim)
    ...
    public:
        ImplicitBox(const VectorDr & minCorner, const VectorDr & lengths):
            _center(minCorner + lengths / 2), _halfLengths(lengths / 2) {}
    ...
    template<int Dim> class ImplicitPlane final : public ImplicitSurface<Dim> {
        DECLARE_DIM_TYPES(Dim)
    ...
    public:
        ImplicitPlane(const VectorDr & position, const VectorDr & direction):
            _position(position), _normal(direction.normalized()) {}
    ...
    template<int Dim> class ImplicitEllipsoid final : public ImplicitSurface<Dim> {
        DECLARE_DIM_TYPES(Dim)
    ...
    public:
        ...
        ImplicitEllipsoid(const VectorDr & center, const VectorDr & semiAxels):
            _center(center), _semiAxels(semiAxels) {}
        virtual ~ImplicitEllipsoid() = default;

        virtual VectorDr closestPosition(const VectorDr & pos) const override {
            return pos / pos.cwiseQuotient(_semiAxels).norm();
        } // not accurate solution
        virtual VectorDr closestNormal(const VectorDr & pos) const override {
            return (pos - closestPosition(pos)).normalized() * (isInside(pos) ? -1 : 1);
        }
        virtual real distance(const VectorDr & pos) const override { return (pos - closestPosition(pos)).norm(); }
        virtual real signedDistance(const VectorDr & pos) const override {
            return distance(pos) * (isInside(pos) ? -1 : 1);
        }
        virtual bool isInside(const VectorDr & pos) const override {
            return isInside(pos.cwiseQuotient(_semiAxels).squaredNorm() - 1);
        }
    };
