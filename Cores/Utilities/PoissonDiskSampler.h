...
namespace PhysX {
    ...
    public:
        PoissonDiskSampler(VectorDr boundary, VectorDr center, real radius)
            ...
            up_bound(center + boundary), low_bound(center - boundary),
            radius(radius) {
        ...
        std::vector<VectorDr> generatePoints(){
            ...
            while (!processList.empty()) {
                static std::uniform_int_distribution<> disI(0, processList.size() - 1);
                int idx = disI(gen);
            ...
            for (const auto & idx : grid){
                if (idx != -1){
                    samples.push_back(gridPoints[idx]);
                }
            }
            return samples;
        }
        ...
        bool isInBounds(const VectorDr& point) const {
            ...
            if constexpr(Dim == 2)
                return grid_idx.x() >= 0 && grid_idx.x() < grid_size.x() && grid_idx.y() >= 0 && grid_idx.y() < grid_size.y();
            else if constexpr (Dim == 3)
                return grid_idx.x() >= 0 && grid_idx.x() < grid_size.x() && grid_idx.y() >= 0 && grid_idx.y() < grid_size.y() && grid_idx.z() >= 0 && grid_idx.z() < grid_size.z();
        }
    ...
    private:
        ...
        bool isFarEnough(const VectorDr& point) const {
            ...
            if constexpr (Dim == 2){
                for (int y = std::max(0, grid_idx.y() - 3); y <= std::min(grid_size.y() - 1, grid_idx.y() + 3); ++y) {
                    for (int x = std::max(0, grid_idx.x() - 3); x <= std::min(grid_size.x() - 1, grid_idx.x() + 3); ++x) {
                ...
                for (int z = std::max(0, grid_idx.z() - 2); z <= std::min(grid_size.z() - 1, grid_idx.z() + 2); ++z){
                    for (int y = std::max(0, grid_idx.y() - 2); y <= std::min(grid_size.y() - 1, grid_idx.y() + 2); ++y) {
                        for (int x = std::max(0, grid_idx.x() - 2); x <= std::min(grid_size.x() - 1, grid_idx.x() + 2); ++x) {
        ...
        void addToGrid(const VectorDr& point) {
            ...
            if constexpr (Dim == 2){
                ...
                grid[grid_idx.z() * grid_size.y() * grid_size.x() + grid_idx.y() * grid_size.x() + grid_idx.x()] = gridPoints.size();
                gridPoints.push_back(point);
            }
        }
    };
